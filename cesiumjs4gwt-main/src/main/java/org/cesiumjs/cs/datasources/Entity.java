...
public class Entity {
  ...
  /**
   * Gets whether this entity is being displayed, taking into account the
   * visibility of any ancestor entities.
   */
  @JsProperty
  ...
  /**
   * Entity instances aggregate multiple forms of visualization into a single
   * high-level object. They can be created manually and added to Viewer#entities
   * or be produced by data sources, such as CzmlDataSource and GeoJsonDataSource.
   */
  @JsConstructor
  public Entity() {
  }
  ...
  /**
   * Checks if the given Scene supports materials besides Color on Entities draped
   * on terrain. If this feature is not supported, Entities with non-color
   * materials but no `height` will instead be rendered as if height is 0.
   * 
   * @param scene The current scene.
   * @return Whether or not the current scene supports materials for entities on
   *         terrain.
   */
  @JsMethod
  ...
  /**
   * Checks if the given Scene supports polylines clamped to the ground.. If this
   * feature is not supported, Entities with PolylineGraphics will be rendered
   * with vertices at the provided heights and using the `followSurface` parameter
   * instead of clamped to the ground.
   * 
   * @param scene The current scene.
  ...
  /**
   * Adds a property to this object. Once a property is added, it can be observed
   * with {@link #definitionChanged()} and composited with
   * {@link org.cesiumjs.cs.collections.CompositeEntityCollection}
   * 
   ...
   * Returns undefined if orientation or position are undefined.
   * 
   ...
   *         not provided. Result is undefined if position or orientation are
   *         undefined.
   */
  @JsMethod
   ...
   * @return true if the object should have data during the provided time, false
   *         otherwise.
   */
  @JsMethod
