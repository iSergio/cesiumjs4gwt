...
public class BoundingSphere {
  /**
   * The center point of the sphere. Default: {@link Cartesian3#ZERO()}
   */
  @JsProperty
   ...
   * A bounding sphere with a center and a radius.
   * 
   * @param center The center of the bounding sphere. Default:
   ...
   * @return The modified result parameter or a new BoundingSphere instance if
   *         none was provided. (Returns undefined if sphere is undefined)
   */
  @JsMethod
  ...
  /**
   * The distances calculated by the vector from the center of the bounding sphere
   * to position projected onto direction plus/minus the radius of the bounding
   * sphere.
   * 
  ...
  /**
   * Compares the provided BoundingSphere componentwise and returns true if they
  ...
  /**
   * Computes a bounding sphere by enlarging the provided sphere to contain the
   * provided point.
   * 
  ...
  /**
   * Computes a tight-fitting bounding sphere enclosing the provided array of
   * bounding spheres.
   * 
  ...
  /**
   * Computes a bounding sphere from the corner points of an axis-aligned bounding
   * box. The sphere tighly and fully encompases the box.
   * 
   * <pre>
   * Example: {
   *   &#64;code
   *   // Create a bounding sphere around the unit cube
   *   BoundingSphere sphere = BoundingSphere.fromCornerPoints(new Cartesian3(-0.5, -0.5, -0.5),
   ...
   * EncodedCartesian3s, where the points are stored in parallel flat arrays in X,
   * Y, Z, order. The bounding sphere is computed by running two algorithms, a
   * naive algorithm and Ritter's algorithm. The smaller of the two spheres is
   * used to ensure a tight fit.
   * 
   * @param positionsHigh An array of high bits of the encoded cartesians that the
   *                      bounding sphere will enclose. Each point is formed from
   *                      three elements in the array in the order X, Y, Z.
   * @param positionsLow  An array of low bits of the encoded cartesians that the
   *                      bounding sphere will enclose. Each point is formed from
   *                      three elements in the array in the order X, Y, Z.
   * @return The modified result parameter or a new BoundingSphere instance if one
   *         was not provided.
   * @see <a href=
   *      "http://blogs.agi.com/insight3d/index.php/2008/02/04/a-bounding/">Sphere
   *      computation article</a>
   */
  @JsMethod
  ...
  /**
   * Computes a tight-fitting bounding sphere enclosing the provided oriented
   * bounding box.
   * 
  ...
  /**
   * Computes a tight-fitting bounding sphere enclosing a list of 3D Cartesian
   * points. The bounding sphere is computed by running two algorithms, a naive
   * algorithm and Ritter's algorithm. The smaller of the two spheres is used to
   * ensure a tight fit.
   * 
   * @param positions An array of points that the bounding sphere will enclose.
   *                  Each point must have x, y, and z properties.
   * @param result    The object onto which to store the result.
   * @return The modified result parameter or a new BoundingSphere instance if one
   *         was not provided.
   * @see <a href=
   *      "http://blogs.agi.com/insight3d/index.php/2008/02/04/a-bounding/">Bounding
   *      Sphere computation article</a>
   */
  @JsMethod
  ...
  /**
   * Computes a bounding sphere from an rectangle in 3D. The bounding sphere is
   * created using a subsample of points on the ellipsoid and contained in the
   * rectangle. It may not be accurate for all rectangles on all types of
   * ellipsoids.
   * 
   ...
   * 
   * @param rectangle     The valid rectangle used to create a bounding sphere.
   * @param ellipsoid     The ellipsoid used to determine positions of the
   *                      rectangle. Default: {@link Ellipsoid#WGS84()}
  ...
  /**
   * Computes a bounding sphere from an rectangle projected in 2D. The bounding
   * sphere accounts for the object's minimum and maximum heights over the
   * rectangle.
   * 
   * @param rectangle     The rectangle around which to create a bounding sphere.
   * @param projection    The projection used to project the rectangle into 2D.
   ...
   * the points are stored in a flat array in X, Y, Z, order. The bounding sphere
   * is computed by running two algorithms, a naive algorithm and Ritter's
   * algorithm. The smaller of the two spheres is used to ensure a tight fit.
   * 
   * @param positions An array of points that the bounding sphere will enclose.
   *                  Each point is formed from three elements in the array in the
   *                  order X, Y, Z.
   * @return The modified result parameter or a new BoundingSphere instance if one
   *         was not provided.
   * @see <a href=
   ...
   * Example: {
   *   &#64;code
   *   // Compute the bounding sphere from 3 positions, each specified relative to
   *   // a center.
   *   // In addition to the X, Y, and Z coordinates, the points array contains
   *   // two additional
   *   // elements per point which are ignored for the purpose of computing the
   *   // bounding sphere.
   ...
   *                  Each point is formed from three elements in the array in the
   *                  order X, Y, Z.
   * @param center    The position to which the positions are relative, which need
   *                  not be the origin of the coordinate system. This is useful
   *                  when the positions are to be used for relative-to-center
   *                  (RTC) rendering. Default: {@link Cartesian3#ZERO()}
   * @param stride    The number of array elements per vertex. It must be at least
   *                  3, but it may be higher. Regardless of the value of this
   *                  parameter, the X coordinate of the first position is at
   *                  array index 0, the Y coordinate is at array index 1, and the
   *                  Z coordinate is at array index 2. When stride is 3, the X
   *                  coordinate of the next position then begins at array index
   *                  3. If the stride is 5, however, two array elements are
   *                  skipped and the next position begins at array index 5.
   *                  Default: 0
   * @return The modified result parameter or a new BoundingSphere instance if one
   *         was not provided.
   * @see <a href=
   ...
   * @return {@link Intersect#INSIDE()} if the entire sphere is on the side of the
   *         plane the normal is pointing, {@link Intersect#OUTSIDE()} if the
   *         entire sphere is on the opposite side, and
  ...
  /**
   * Creates a bounding sphere in 2D from a bounding sphere in 3D world
   * coordinates.
   * 
   ...
   * 
   * @param sphere     The bounding sphere to transform to 2D.
   * @param projection The projection to 2D. Default:
   *                   {@link org.cesiumjs.cs.core.projection.GeographicProjection}
   * @return The modified result parameter or a new BoundingSphere instance if
   *         none was provided.
   */
  ...
  /**
   * Applies a 4x4 affine transformation matrix to a bounding sphere where there
   * is no scale The transformation matrix is not verified to have a uniform scale
   * of 1. This method is faster than computing the general bounding sphere
   * transform using BoundingSphere.transform.
   * 
   * <pre>
   * Example: {
   *   &#64;code
  ...
  /**
   * Computes a bounding sphere that contains both the left and right bounding
   * spheres.
   * 
   ...
   * The distances calculated by the vector from the center of the bounding sphere
   * to position projected onto direction plus/minus the radius of the bounding
   * sphere. If you imagine the infinite number of planes with normal direction,
   * this computes the smallest distance to the closest and farthest planes from
   * position that intersect the bounding sphere.
   * 
   * @param position  The position to calculate the distance from.
   * @param direction The direction from position.
   * @return The nearest and farthest distances on the bounding sphere from
  ...
  /**
   * Compares this BoundingSphere against the provided BoundingSphere
   * componentwise and returns true if they are equal, false otherwise.
   * 
