...
public class GoogleEarthEnterpriseTerrainDataOptions {
  ...
  ArrayBuffer buffer;
  /**
   * A bit mask indicating which of this tile's four children exist. If a child's
   * bit is set, geometry will be requested for that tile as well when it is
   * needed. If the bit is cleared, the child tile is not requested and geometry
   * is instead upsampled from the parent. The bit values are as follows: Default:
   * 15.
   */
  int childTileMask;
  /**
   * True if this instance was created by upsampling another instance; otherwise,
   * false. Default: false.
   */
