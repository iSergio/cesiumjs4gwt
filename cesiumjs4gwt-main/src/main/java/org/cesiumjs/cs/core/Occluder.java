...
public class Occluder {
  ...
  /**
   * Creates an Occluder derived from an object's position and radius, as well as
   * the camera position. The occluder can be used to determine whether or not
   * other objects are visible or hidden behind the visible horizon defined by the
   * occluder and camera position.
   * 
   * <pre>
   * Example: {
   *   &#64;code
   *   // Construct an occluder one unit away from the origin with a radius of
   *   // one.
  ...
  /**
   * Computes a point that can be used as the occludee position to the visibility
   * functions. Use a radius of zero for the occludee radius. Typically, a user
   * computes a bounding sphere around an object that is used for visibility;
   * however it is also possible to compute a point that if seen/not seen would
   * also indicate if an object is visible/not visible. This function is better
   * called for objects that do not move relative to the occluder and is large,
   * such as a chunk of terrain. You are better off not calling this and using the
   * object's bounding sphere for objects such as a satellite or ground vehicle.
   * 
   * <pre>
   * {
   *   &#64;code
   ...
   * 
   * @param occluderBoundingSphere The bounding sphere surrounding the occluder.
   * @param occludeePosition       List of altitude points on the horizon near the
   *                               surface of the occluder.
   * @param positions              An object containing two attributes:
   *                               occludeePoint and valid which is a boolean
   *                               value.
   * @return An object containing two attributes: occludeePoint and valid which is
   *         a boolean value.
   */
  @JsMethod
  public static native Object computeOccludeePoint(BoundingSphere occluderBoundingSphere, Cartesian3 occludeePosition,
      Cartesian3[] positions);

  /**
   * Computes a point that can be used as the occludee position to the visibility
   * functions from an rectangle.
   * 
   ...
   * @param occluderBoundingSphere The bounding sphere surrounding the occluder.
   * @param cameraPosition         The coordinate of the viewer/camera.
   * @return The occluder derived from an object's position and radius, as well as
   *         the camera position.
   */
  @JsMethod
  ...
  /**
   * Determine to what extent an occludee is visible (not visible, partially
   * visible, or fully visible).
   * 
   * <pre>
   * Example: {
   *   &#64;code
   ...
   * @return {@link Visibility#NONE()} if the occludee is not visible,
   *         {@link Visibility#PARTIAL()} if the occludee is partially visible, or
   *         {@link Visibility#FULL()} if the occludee is fully visible.
   */
  ...
  /**
   * Determines whether or not a sphere, the occludee, is hidden from view by the
   * occluder.
   * 
   * <pre>
   * Example: {
  ...
  /**
   * etermines whether or not a point, the occludee, is hidden from view by the
   * occluder.
   * 
   * <pre>
   * Example: {
