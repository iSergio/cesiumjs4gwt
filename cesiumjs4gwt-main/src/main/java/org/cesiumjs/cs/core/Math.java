...
public class Math {
  ...
  /**
   * The mean radius of the moon, according to the "Report of the IAU/IAG Working
   * Group on Cartographic Coordinates and Rotational Elements of the Planets and
   * satellites: 2000", Celestial Mechanics 82: 83-110, 2002.
   */
  ...
  /**
   * Computes Math.acos(value), but first clamps value to the range [-1.0, 1.0] so
   * that the function will never return NaN.
   * 
   * @param value The value for which to compute acos.
   * @return The acos of the value if the value is in the range [-1.0, 1.0], or
   *         the acos of -1.0 or 1.0, whichever is closer, if the value is outside
   *         the range.
   */
  @JsMethod
  ...
  /**
   * Computes Math.asin(value), but first clamps value to the range [-1.0, 1.0] so
   * that the function will never return NaN.
   * 
   * @param value The value for which to compute asin.
   * @return The asin of the value if the value is in the range [-1.0, 1.0], or
   *         the asin of -1.0 or 1.0, whichever is closer, if the value is outside
   *         the range.
   */
  @JsMethod
  ...
  /**
   * Finds the chord length between two points given the circle's radius and the
   ...
   * [-Math.PI/2, Math.PI/2). Useful for sanitizing data before use in objects
   * requiring correct range.
   * 
   * @param angle The latitude value, in radians, to clamp to the range
   *              [-Math.PI/2, Math.PI/2).
  ...
  /**
   * Returns the hyperbolic cosine of a number. The hyperbolic cosine of value is
   * defined to be (ex + e-x)/2.0 where e is Euler's number, approximately
   * 2.71828183. Special cases: If the argument is NaN, then the result is NaN. If
  ...
  /**
   * Determines if two values are equal using an absolute or relative tolerance
   * test. This is useful to avoid problems due to roundoff error when comparing
   ...
   * @param relativeEpsilon The maximum inclusive delta between left and right for
   *                        the relative tolerance test.
   ...
   * @param relativeEpsilon The maximum inclusive delta between left and right for
   *                        the relative tolerance test.
   * @param absoluteEpsilon Default: relativeEpsilon. The maximum inclusive delta
   *                        between left and right for the absolute tolerance
   *                        test.
   * @return true if the values are equal within the epsilon; otherwise, false.
   */
  @JsMethod
   ...
   * @return The factorial of the provided number or undefined if the number is
   *         less than 0.
   * @see <a href="http://en.wikipedia.org/wiki/Factorial">Factorial on
   *      Wikipedia</a>
   */
  @JsMethod
  ...
  /**
   * Converts a SNORM value in the range [0, rangeMax] to a scalar in the range
   * [-1.0, 1.0].
   * 
  ...
  /**
   * Increments a number with a wrapping to a minimum value if the number exceeds
   * the maximum value.
   * 
   * <pre>
   * Example: {
   ...
   * @param maximumValue The maximum incremented value before rolling over to the
   *                     minimum value.
   * @param minimumValue The number reset to after the maximum value has been
   *                     exceeded.
  ...
  /**
   * Produces an angle in the range -Pi <= angle <= Pi which is equivalent to the
   * provided angle.
   * 
  ...
  /**
   * Computes the next power-of-two integer greater than or equal to the provided
   * positive integer.
   * 
   * <pre>
   * Example:
  ...
  /**
   * Generates a random number in the range of [0.0, 1.0) using a Mersenne
   * twister.
   * 
  ...
  /**
   * Converts a scalar value in the range [rangeMinimum, rangeMaximum] to a scalar
   * in the range [0.0, 1.0]
   * 
   * @param value        The scalar value in the range [rangeMinimum,
   *                     rangeMaximum]
   * @param rangeMinimum The minimum value in the mapped range.
   * @param rangeMaximum The maximum value in the mapped range.
   * @return A scalar value, where rangeMinimum maps to 0.0 and rangeMaximum maps
   *         to 1.0.
   */
  @JsMethod
   ...
   * negative. This is similar to {@link #sign} except that returns 1.0 instead of
  ...
  /**
   * Returns the hyperbolic sine of a number. The hyperbolic sine of value is
   * defined to be (ex - e-x)/2.0 where e is Euler's number, approximately
   * 2.71828183. Special cases: If the argument is NaN, then the result is NaN. If
  ...
  /**
   * Converts a scalar value in the range [-1.0, 1.0] to a SNORM in the range [0,
   * rangeMax]
   * 
  ...
  /**
   * Produces an angle in the range 0 <= angle <= 2Pi which is equivalent to the
   * provided angle.
   * 
   * @param angle in radians
