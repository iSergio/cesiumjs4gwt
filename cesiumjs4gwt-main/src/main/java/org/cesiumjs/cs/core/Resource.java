...
import org.cesiumjs.cs.core.options.DerivedResourceOptions;
import org.cesiumjs.cs.core.options.RequestOptions;
import org.cesiumjs.cs.core.options.ResourceHttpOptions;
import org.cesiumjs.cs.core.options.ResourceImageOptions;
import org.cesiumjs.cs.core.options.ResourceJsonpOptions;
import org.cesiumjs.cs.core.options.ResourceOptions;
import org.cesiumjs.cs.core.options.ResourcePathOptions;
import org.cesiumjs.cs.core.options.ResourcePutOptions;
...
public class Resource {
  ...
  /**
   * True if the Resource has request headers. This is equivalent to checking if
   * the headers property has any keys.
   */
  @JsProperty
  ...
  /**
   * Function to call when a request for this resource fails. If it returns true
   * or a Promise that resolves to true, the request will be retried.
   */
  @JsProperty
  ...
  /**
   * The url to the resource with template values replaced, query string appended
   * and encoded by proxy if one was set.
   */
  @JsProperty
  public String url;
   ...
   *         Returns undefined if request.throttle is true and the request does
   *         not have high enough priority.
   */
  @JsMethod
  ...
  /**
   * Combines the specified object and the existing query parameters. This allows
   * you to add many parameters at once, as opposed to adding them one at a time
   * to the queryParameters property.
   * 
   ...
   * 
   * @return The modified result parameter or a new Resource instance if one was
   *         not provided.
   */
  @JsMethod
  ...
  /**
   * Asynchronously loads the resource as raw binary data. Returns a promise that
   * will resolve to an ArrayBuffer once loaded, or reject if the resource failed
   * to load. The data is loaded using XMLHttpRequest, which means that in order
   * to make requests to another origin, the server must have Cross-Origin
   * Resource Sharing (CORS) headers enabled.
   * 
   * @return a promise that will resolve to the requested data when loaded.
   *         Returns undefined if request.throttle is true and the request does
   *         not have high enough priority.
  ...
  /**
   * Asynchronously loads the given image resource. Returns a promise that will
   * resolve to an Image once loaded, or reject if the image failed to load.
   * 
   * @return a promise that will resolve to the requested data when loaded.
   *         Returns undefined if request.throttle is true and the request does
   *         not have high enough priority.
  ...
  /**
   * Asynchronously loads the given resource as JSON. Returns a promise that will
   * resolve to a JSON object once loaded, or reject if the resource failed to
   * load. The data is loaded using XMLHttpRequest, which means that in order to
   * make requests to another origin, the server must have Cross-Origin Resource
   * Sharing (CORS) headers enabled. This function adds 'Accept:
   * application/json,*\/*;q=0.01' to the request headers, if not already
   * specified.
   * 
   * @return a promise that will resolve to the requested data when loaded.
   *         Returns undefined if request.throttle is true and the request does
   ...
   * Requests a resource using JSONP.
   * 
   * @param callbackParameterName The callback parameter name that the server
   *                              expects.
   * @return a promise that will resolve to the requested data when loaded.
   *         Returns undefined if request.throttle is true and the request does
   *         not have high enough priority.
  ...
  /**
   * Asynchronously loads the given resource as text. Returns a promise that will
   * resolve to a String once loaded, or reject if the resource failed to load.
   * The data is loaded using XMLHttpRequest, which means that in order to make
   * requests to another origin, the server must have Cross-Origin Resource
   * Sharing (CORS) headers enabled.
   * 
   * @return a promise that will resolve to the requested data when loaded.
   *         Returns undefined if request.throttle is true and the request does
   *         not have high enough priority.
   ...
   * Returns the base path of the Resource.
   * 
   * @param includeQuery Whether or not to include the query string and fragment
   *                     form the uri
   * @return The base URI of the resource
   */
  @JsMethod
  ...
  /**
   * Returns a resource relative to the current instance. All properties remain
   * the same as the current instance unless overridden in options.
   * 
  ...
  /**
   * Asynchronously gets headers the given resource. Returns a promise that will
   * resolve to the result once loaded, or reject if the resource failed to load.
   * The data is loaded using XMLHttpRequest, which means that in order to make
   * requests to another origin, the server must have Cross-Origin Resource
   * Sharing (CORS) headers enabled.
   * 
   ...
   * @see <a href="http://wiki.commonjs.org/wiki/Promises/A">CommonJS
   *      Promises/A</a>
   */
  @JsMethod
  ...
  /**
   * Asynchronously gets options the given resource. Returns a promise that will
   * resolve to the result once loaded, or reject if the resource failed to load.
   * The data is loaded using XMLHttpRequest, which means that in order to make
   * requests to another origin, the server must have Cross-Origin Resource
   * Sharing (CORS) headers enabled.
  ...
  /**
   * Asynchronously posts data the given resource. Returns a promise that will
   * resolve to the result once loaded, or reject if the resource failed to load.
   * The data is loaded using XMLHttpRequest, which means that in order to make
   * requests to another origin, the server must have Cross-Origin Resource
   * Sharing (CORS) headers enabled.
   ...
   * 
   * @return A promise to a boolean, that if true will cause the resource request
   *         to be retried.
   */
  @JsMethod
   ...
   * Combines the specified object and the existing query parameters. This allows
   * you to add many parameters at once, as opposed to adding them one at a time
   * to the queryParameters property. If a value is already set, it will be
   ...
   * 
   * @param params       The query parameters
   * @param useAsDefault If true the params will be used as the default values, so
   *                     they will only be set if they are undefined.
   */
  @JsMethod
  public native void setQueryParameters(JsObject params, boolean useAsDefault);

  /**
   * Combines the specified object and the existing template values. This allows
   * you to add many values at once, as opposed to adding them one at a time to
   * the templateValues property. If a value is already set, it will become an
   * array and the new value will be appended.
   * 
   ...
   * 
   * @param template     The template values
   * @param useAsDefault If true the values will be used as the default values, so
   *                     they will only be set if they are undefined. Default:
   *                     false
   */
  @JsMethod
  ...
  public interface RetryCallback {
     ...
     * @return If true or a promise that resolved to true, the resource will be
     *         retried. Otherwise the failure will be returned.
     */
