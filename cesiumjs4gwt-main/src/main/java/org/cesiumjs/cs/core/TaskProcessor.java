 ...
 * via a promise. The Worker is not constructed until a task is scheduled.
 *
...
public class TaskProcessor {
     ...
     * A wrapper around a web worker that allows scheduling tasks for a given worker, returning results asynchronously
     * via a promise. The Worker is not constructed until a task is scheduled.
     * @param workerName The name of the worker. This is expected to be a script in the Workers folder.
     */
    @JsConstructor
     ...
     * via a promise. The Worker is not constructed until a task is scheduled.
     * @param workerName The name of the worker. This is expected to be a script in the Workers folder.
     * @param maximumActiveTasks The maximum number of active tasks. Once exceeded, scheduleTask will not
     *                           queue any more tasks, allowing work to be rescheduled in future frames.
     */
    @JsConstructor
     ...
     * @return A promise that resolves to the result when the web worker has loaded and compiled the web assembly
     * module and is ready to process tasks.
     */
    @JsMethod
     ...
     *
     * If this object was destroyed, it should not be used; calling any function other than isDestroyed will result
     * in a DeveloperError exception.
    ...
    /**
     * Schedule a task to be processed by the web worker asynchronously. If there are currently more tasks active
     * than the maximum set by the constructor, will immediately return undefined. Otherwise, returns a promise
     * that will resolve to the result posted back by the worker when finished.
     * @param parameters Any input data that will be posted to the worker.
     * @return Either a promise that will resolve to the result when available, or undefined if there are too many active tasks,
     */
    @JsMethod
     ...
     * that will resolve to the result posted back by the worker when finished.
     * @param parameters Any input data that will be posted to the worker.
     * @param transferableObjects An array of objects contained in parameters that should be transferred to the worker instead of copied.
     * @return Either a promise that will resolve to the result when available, or undefined if there are too many active tasks,
     */
    @JsMethod
