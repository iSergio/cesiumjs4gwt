...
public class PointPrimitiveCollection extends Collection<PointPrimitive> {
  ...
  /**
   * A renderable collection of points.
   * Points are added and removed from the collection using {@link #add} and
   * {@link #remove}.
   * Performance:
   * For best performance, prefer a few collections, each with many points, to
   * many collections with only a few points each. Organize collections so that
   * points with the same update frequency are in the same collection, i.e.,
   * points that do not change should be in one collection; points that change
   * every frame should be in another collection; and so on.
   */
  @JsConstructor
  ...
  /**
   * Creates and adds a point with the specified initial properties to the
   * collection. The added point is returned so it can be modified or removed from
   * the collection later.
   * Performance: Calling add is expected constant time. However, the collection's
   * vertex buffer is rewritten - an O(n) operation that also incurs CPU to GPU
   * overhead. For best performance, add as many pointPrimitives as possible
