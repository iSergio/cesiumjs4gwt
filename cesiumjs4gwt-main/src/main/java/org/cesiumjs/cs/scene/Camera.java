...
import org.cesiumjs.cs.core.OrthographicFrustum;
import org.cesiumjs.cs.core.PerspectiveFrustum;
import org.cesiumjs.cs.core.PerspectiveOffCenterFrustum;
...
public class Camera {
  /**
   * The default heading/pitch/range that is used when the camera flies to a
   * location that contains a bounding sphere.
   */
  @JsProperty
  public static HeadingPitchRange DEFAULT_OFFSET;
  /**
   * A scalar to multiply to the camera position and add it back after setting the
   * camera to view the rectangle. A value of zero means the camera will view the
   * entire {@link Camera#DEFAULT_VIEW_RECTANGLE}, a value greater than zero will
   * move it further away from the extent, and a value less than zero will move it
   * close to the extent.
   */
  @JsProperty
  ...
  /**
   * If set, the camera will not be able to rotate past this axis in either
   * direction. Default Value: undefined
   */
  @JsProperty
  public Cartesian3 constrainedAxis;
  /**
   * The default amount to rotate the camera when an argument is not provided to
   * the look methods. Default Value: Math.PI / 60.0
   */
  @JsProperty
  public double defaultLookAmount;
  /**
   * The default amount to move the camera when an argument is not provided to the
   * move methods. Default: 100000.0;
   */
  @JsProperty
  public double defaultMoveAmount;
  /**
   * The default amount to rotate the camera when an argument is not provided to
   * the rotate methods. Default: Math.PI / 3600.0
   */
  @JsProperty
  public double defaultRotateAmount;
  /**
   * The default amount to move the camera when an argument is not provided to the
   * zoom methods. Default: 100000.0;
   */
  @JsProperty
  ...
  /**
   * The region of space in view. Default: PerspectiveFrustum()
   * 
  ...
  /**
   * Gets the inverse camera transform. Default: Matrix4.IDENTITY
   */
  ...
  /**
   * The factor multiplied by the the map size used to determine where to clamp
   * the camera position when zooming out from the surface. The default is 1.5.
   * Only valid for 2D and the map is rotatable. Default: 1.5
   */
  @JsProperty
  ...
  /**
   * The amount the camera has to change before the changed event is raised. The
   * value is a percentage in the [0, 1] range. Default: 0.5
   */
  @JsProperty
   ...
   * expressed in radians and height in meters. In 2D and Columbus View, it is
   * possible for the returned longitude and latitude to be outside the range of
   * valid longitudes and latitudes when the camera is outside the map.
   */
  ...
  /**
   * Gets the camera's reference frame. The inverse of this transformation is
   * appended to the view matrix. Default: Matrix4.IDENTITY
   */
   ...
   *
   * The orientation forms an orthonormal basis with a view, up and right = view x
   * up unit vectors.
   *
  ...
  /**
   * Transform a vector or point from the camera's reference frame to world
   * coordinates.
   * 
  ...
  /**
   * Cancels the current camera flight if one is in progress. The camera is left
  ...
  /**
   * Completes the current camera flight and moves the camera immediately to its
   ...
   * 
   * @return The visible rectangle or undefined if the ellipsoid isn't visible at
   *         all.
   */
  @JsMethod
  ...
  /**
   * Fly the camera to the home view. Use {@link Camera#DEFAULT_VIEW_RECTANGLE} to
   * set the default view for the 3D scene. The home view for 2D and columbus view
   ...
   * shows the entire map.
   * 
   * @param duration The number of seconds to complete the camera flight to
   *                 home. @see #flyTo
   */
  @JsMethod
  ...
  /**
   * Flies the camera to a location where the current view contains the provided
   * bounding sphere. The offset is heading/pitch/range in the local east-north-up
   * reference frame centered at the center of the bounding sphere. The heading
   * and the pitch angles are defined in the local east-north-up reference frame.
   * The heading is the angle from y axis and increasing towards the x axis. Pitch
   * is the rotation from the xy-plane. Positive pitch angles are below the plane.
   * Negative pitch angles are above the plane. The range is the distance from the
   * center. If the range is zero, a range will be computed such that the whole
   * bounding sphere is visible.
   *
   * In 2D and Columbus View, there must be a top down view. The camera will be
   * placed above the target looking down. The height above the target will be the
   * range. The heading will be aligned to local north.
   * 
  ...
  /**
   * Gets the magnitude of the camera position. In 3D, this is the vector
   * magnitude. In 2D and Columbus view, this is the distance to the map.
   * 
  ...
  /**
   * Create a ray from the camera position through the pixel at windowPosition in
   * world coordinates.
   * 
   ...
   */
  @JsMethod
  public native double getPixelSize(BoundingSphere boundingSphere, double drawingBufferWidth,
   ...
   * 
   * @param axis  The axis to rotate around.
   * @param angle The angle, in radians, to rotate by. Defaults to
   *              defaultLookAmount.
   * @see #lookUp
   * @see #lookDown
   * @see #lookLeft
  ...
  // TODO: Example
  /**
   * Sets the camera position and orientation using a target and offset. The
   * target must be given in world coordinates. The offset can be either a
   * cartesian or heading/pitch/range in the local east-north-up reference frame
   * centered at the target. If the offset is a cartesian, then it is an offset
   * from the center of the reference frame defined by the transformation matrix.
   * If the offset is heading/pitch/range, then the heading and the pitch angles
   * are defined in the reference frame defined by the transformation matrix. The
   * heading is the angle from y axis and increasing towards the x axis. Pitch is
   * the rotation from the xy-plane. Positive pitch angles are below the plane.
   * Negative pitch angles are above the plane. The range is the distance from the
   * center. In 2D, there must be a top down view. The camera will be placed above
   * the target looking down. The height above the target will be the magnitude of
   * the offset. The heading will be determined from the offset. If the heading
   * cannot be determined from the offset, the heading will be north.
   * 
   * @param target The target position in world coordinates.
   * @param offset The offset from the target in the local east-north-up reference
   *               frame centered at the target.
   */
  @JsMethod
  ...
  // TODO: Example
  /**
   * Sets the camera position and orientation using a target and transformation
   * matrix. The offset can be either a cartesian or heading/pitch/range. If the
   * offset is a cartesian, then it is an offset from the center of the reference
   * frame defined by the transformation matrix. If the offset is
   * heading/pitch/range, then the heading and the pitch angles are defined in the
   * reference frame defined by the transformation matrix. The heading is the
   * angle from y axis and increasing towards the x axis. Pitch is the rotation
   * from the xy-plane. Positive pitch angles are below the plane. Negative pitch
   * angles are above the plane. The range is the distance from the center. In 2D,
   * there must be a top down view. The camera will be placed above the center of
   * the reference frame. The height above the target will be the magnitude of the
   * offset. The heading will be determined from the offset. If the heading cannot
   * be determined from the offset, the heading will be north.
   * 
   ...
   * 
   * @param transform The transformation matrix defining the reference frame.
   * @param offset    The offset from the target in a reference frame centered at
   *                  the target.
   */
  @JsMethod
  ...
  /**
   * Rotates the camera around its right vector by amount, in radians, in the
   ...
   * opposite direction of its up vector.
   * 
   * @param amount The amount, in radians, to rotate by. Default:
   *               {@link #defaultLookAmount}
   * @see #lookUp()
   */
  @JsMethod
  ...
  /**
   * Rotates the camera around its up vector by amount, in radians, in the
   * opposite direction of its right vector.
   * 
   * @see #lookRight()
   */
  @JsMethod
   ...
   * 
   * @param direction The direction to move.
   * @param amount    The amount, in meters, to move. Defaults to
   *                  defaultMoveAmount.
   * @see #moveBackward()
   * @see #moveForward()
   * @see #moveLeft()
  ...
  /**
   * Translates the camera's position by amount along the opposite direction of
   ...
   * 
   * @param windowPosition The x and y coordinates of a pixel.
   * @return If the ellipsoid or map was picked, returns the point on the surface
   *         of the ellipsoid or map in world coordinates. If the ellipsoid or map
   *         was not picked, returns undefined.
   */
  @JsMethod
   ...
   * 
   * @param windowPosition The x and y coordinates of a pixel.
   * @param ellipsoid      The ellipsoid to pick. Default:
  ...
  /**
   * Rotates the camera around axis by angle. The distance of the camera's
   * position to the center of the camera's reference frame remains the same.
   * 
  ...
  /**
   * Rotates the camera around the center of the camera's reference frame by angle
   * downwards.
   * 
   * @see #rotateUp(double)
  ...
  /**
   * Switches the frustum/projection to orthographic. This function is a no-op in
   * 2D which will always be orthographic.
   */
  @JsMethod
  public native void switchToOrthographicFrustum();

  /**
   * Switches the frustum/projection to perspective. This function is a no-op in
   * 2D which must always be orthographic.
   */
  @JsMethod
  public native void switchToPerspectiveFrustum();

  /**
   * Rotate the camera counter-clockwise around its direction vector by amount, in
   * radians.
   * 
   * @see #twistRight()
   */
   ...
   * radians.
   * 
   * @param amount The amount, in radians, to rotate by. Defaults to
   *               defaultLookAmount.
   * @see #twistRight()
   */
  @JsMethod
  ...
  /**
   * Sets the camera so that the current view contains the provided bounding
   * sphere. The offset is heading/pitch/range in the local east-north-up
   * reference frame centered at the center of the bounding sphere. The heading
   * and the pitch angles are defined in the local east-north-up reference frame.
   * The heading is the angle from y axis and increasing towards the x axis. Pitch
   * is the rotation from the xy-plane. Positive pitch angles are below the plane.
   ...
   * bounding sphere is visible.
   *
   * In 2D, there must be a top down view. The camera will be placed above the
   * target looking down. The height above the target will be the range. The
   * heading will be determined from the offset. If the heading cannot be
   ...
   * 
   * @param boundingSphere The bounding sphere to view, in world coordinates.
   * @param offset         The offset from the target in the local east-north-up
   *                       reference frame centered at the target.
   */
  @JsMethod
  public native void viewBoundingSphere(BoundingSphere boundingSphere, HeadingPitchRange offset);

  /**
   * Transform a vector or point from world coordinates to the camera's reference
   * frame.
   * 
   * @param cartesian The vector or point to transform.
   * @return The transformed vector or point.
