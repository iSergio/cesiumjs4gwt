...
public class EllipsoidSurfaceAppearance extends Appearance {
  /**
   * The VertexFormat that all EllipsoidSurfaceAppearance instances are compatible
   * with, which requires only position and st attributes. Other attributes are
   * procedurally computed in the fragment shader.
   */
  @JsProperty(name = "VERTEX_FORMAT")
  public static native VertexFormat VERTEX_FORMAT();

  /**
   * When true, the geometry is expected to be on the ellipsoid's surface - not at
   * a constant height above it - so EllipsoidSurfaceAppearance#renderState has
   * backface culling enabled. Default: false
   */
   ...
   * {@link EllipsoidSurfaceAppearance#VERTEX_FORMAT()}
   */
   ...
   * {@link PolygonGeometry} and {@link RectangleGeometry}, which supports all
   * materials like {@link MaterialAppearance} with
   * {@link MaterialAppearance.MaterialSupport#ALL()}. However, this appearance
   * requires fewer vertex attributes since the fragment shader can procedurally
