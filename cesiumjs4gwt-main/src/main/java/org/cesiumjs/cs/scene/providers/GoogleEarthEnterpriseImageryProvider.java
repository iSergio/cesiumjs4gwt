...
import org.cesiumjs.cs.scene.providers.options.GoogleEarthEnterpriseImageryProviderOptions;
 ...
 */
@JsType(isNative = true, namespace = "Cesium", name = "GoogleEarthEnterpriseImageryProvider")
public class GoogleEarthEnterpriseImageryProvider implements ImageryProvider {
    /**
     ...
     * This function should not be called before GoogleEarthEnterpriseImageryProvider#ready returns true.
     */
     ...
     * If this property is false, an alpha channel, if present, will be ignored. If this property is true,
     * any images without an alpha channel will be treated as if their alpha is 1.0 everywhere.
     * Setting this property to false reduces memory usage and texture upload time.
     */
     ...
     * GoogleEarthEnterpriseImageryProvider#ready returns true.
     */
    @JsProperty(name = "maximumLevel")
    public native int maximumLevel();
    /**
     * Gets the minimum level-of-detail that can be requested. This function should not be called
     * before GoogleEarthEnterpriseImageryProvider#ready returns true.
     */
     ...
     * too many active requests to the server, and the request should be retried later. The resolved
     * image may be either an Image or a Canvas DOM object.
     */
    @JsMethod
     ...
     * image may be either an Image or a Canvas DOM object.
     */
    @JsMethod(name = "requestImage")
    public native Promise<Canvas, Void> requestCanvas(int x, int y, int level);
}
