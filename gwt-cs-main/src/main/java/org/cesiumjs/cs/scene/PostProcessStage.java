...
public class PostProcessStage implements PostProcess {
    ...
    /**
     * Determines if this post-process stage is ready to be executed. A stage is only executed when both ready and
     * PostProcessStage#enabled are true. A stage will not be ready while it is waiting on textures to load.
     */
    ...
    /**
     * A number in the range (0.0, 1.0] used to scale the output texture dimensions. A scale of 1.0 will render this
     * post-process stage to a texture the size of the viewport.
     */
     ...
     * The object property values can be either a constant or a function. The function will be called each frame
     * before the post-process stage is executed.
     *
     * A constant value can also be a URI to an image, a data URI, or an HTML element that can be used as a texture,
     * such as HTMLImageElement or HTMLCanvasElement.
     *
     * If this post-process stage is part of a PostProcessStageComposite that does not execute in series, the constant
     * value can also be the name of another stage in a composite. This will set the uniform to the output texture
     * the stage with that name.
     */
