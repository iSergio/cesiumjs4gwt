...
public class KML extends AbstractExample {
    private class ViewerPanel implements IsWidget {
        ...
        public void reset() {
            ...
            _csPanelAbstract.getViewer().clock().clockRange = ClockRange.UNBOUNDED();
            _csPanelAbstract.getViewer().clock().clockStep = ClockStep.SYSTEM_CLOCK();
        }
    }
    ...
    public void buildPanel() {
        ...
        kmlLBox.addChangeHandler(new ChangeHandler() {
            ...
            public void onChange(ChangeEvent changeEvent) {
                ...
                final Viewer viewer = csVPanel._csPanelAbstract.getViewer();
                ...
                if (!source.getSelectedValue().contains("bikeRide")) {
                    csVPanel._csPanelAbstract.getViewer().dataSources().add(KmlDataSource.load(source.getSelectedValue(), kmlDataSourceOptions));
                } else {
                    csVPanel._csPanelAbstract.getViewer().dataSources().add(KmlDataSource.load(source.getSelectedValue(), kmlDataSourceOptions)).then(new Fulfill<KmlDataSource>() {
                        @Override
