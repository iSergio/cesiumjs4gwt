...
public class Picking extends AbstractExample {
    ...
    private class ViewerPanel implements IsWidget {
            ...
            if (_csPanelAbstract == null) {
                ...
                _csPanelAbstract = new ViewerPanelAbstract(csConfiguration) {
                    ...
                    public Viewer createViewer(Element element) {
                        final Viewer csViewer = new Viewer(element);
                        showCartographicPositionOnMouseOver(csViewer);
                        return csViewer;
                    }
                };
            }
        ...
        private void pickEntity() {
            ...
            final Entity entity = _csPanelAbstract.getViewer().entities().add(new Entity(entityOptions));

            _handler = new ScreenSpaceEventHandler(_csPanelAbstract.getViewer().scene().canvas());
            _handler.setInputAction(new ScreenSpaceEventHandler.Listener<MouseMoveEvent>() {
                @Override
                public void function(MouseMoveEvent event) {
                    PickedObject pickedObject = _csPanelAbstract.getViewer().scene().pick(event.endPosition);
        ...
        private void drillDownPicking() {
            ...
            Entity red = _csPanelAbstract.getViewer().entities().add(new Entity(entityOptions));
            ...
            entityOptions.polygon = new PolygonGraphics(polygonGraphicsOptions);
            Entity blue = _csPanelAbstract.getViewer().entities().add(new Entity(entityOptions));
            ...
            entityOptions = new EntityOptions();
            entityOptions.polygon = new PolygonGraphics(polygonGraphicsOptions);
            Entity green = _csPanelAbstract.getViewer().entities().add(new Entity(entityOptions));
            ...
            _handler.setInputAction(new ScreenSpaceEventHandler.Listener<MouseMoveEvent>() {
                @Override
                public void function(MouseMoveEvent event) {
                    PickedObject[] pickedObjects = _csPanelAbstract.getViewer().scene().drillPick(event.endPosition);
        ...
        private void pickPosition() {
            ...
            final Entity modelEntity = _csPanelAbstract.getViewer().entities().add(new Entity(entityOptions));
            _csPanelAbstract.getViewer().zoomTo(modelEntity);
            ...
            entityOptions = new EntityOptions();
            entityOptions.label = new LabelGraphics(labelGraphicsOptions);
            final Entity labelEntity = _csPanelAbstract.getViewer().entities().add(new Entity(entityOptions));
            _handler = new ScreenSpaceEventHandler(_csPanelAbstract.getViewer().scene().canvas());
            _handler.setInputAction(new ScreenSpaceEventHandler.Listener<MouseMoveEvent>() {
                @Override
                public void function(MouseMoveEvent event) {
                    ...
                    Scene scene = _csPanelAbstract.getViewer().scene();
                    ...
                    if (scene.pickPositionSupported() && pickedObject != null && pickedObject.id == modelEntity) {
                        Cartesian3 cartesian = _csPanelAbstract.getViewer().scene().pickPosition(event.endPosition);
                        ...
                        if (cartesian != null) {
                            ...
                            labelEntity.label.eyeOffset = new ConstantProperty<>(new Cartesian3(0.0, 0.0, ((PerspectiveFrustum)camera.frustum).near * 1.5 - Cartesian3.distance(cartesian, camera.position)));
    ...
    private class MValueChangeHandler implements ValueChangeHandler<Boolean> {
        ...
        public void onValueChange(ValueChangeEvent<Boolean> valueChangeEvent) {
            ...
            if (source.equals(_showPosOnMouseOverTBtn)) {
                _csVPanel.showCartographicPositionOnMouseOver(_csVPanel.getViewer());
